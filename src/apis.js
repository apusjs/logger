export default function (wpo, undef) {
  // startTime配置兼容
  let startTime = wpo.startTime || wpo.config.startTime
  let scriptStart = startTime
  ...
  const send = function (params, sampling) {
    ...
    params.tag = wpo.config.tag && (wpo.safetyCall(wpo.config.tag, [], wpo.config.tag + '') || '')
   ...
   */
  const filterErrMsg = function (msg, filterPattern) {
    // 空白消息省略
    if (!msg) return true
    if (!filterPattern) return false
    try {
      let type = Object.prototype.toString.call(filterPattern).substring(8).replace(']', '')
